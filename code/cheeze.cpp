...
static r64
image_dist(const image& im, const rgba& q) {
    auto* px = im.px;
    u32 n = im.w * im.h;
    r64 diff = 0;
    for (u32 i = 0; i < n; ++i) {
        diff += pixel_dist(*px++, q);
    }
    return diff;
}
...
static r64
image_dist(const image& im, const fc& q) {
...
static rgba
image_sq_average(const image& im) {
    const u32 w = im.w;
    const u32 h = im.h;
    r64 r,g,b,a, n = w * h;
    rgba* px = im.px;
...
static rgba
image_nelder_mead(const image& im) {
...
static void
solve_block(prog_state& state, const block& r, const id_path& sel, const image& task) {
    if (r.children.size() > 0) {
        for (auto& p : r.children) {
            id_path subsel = sel;
            subsel.push_back(p.id);
            solve_block(state, p, subsel, task);
        }
    }
    else {
        image chunk = {.w=r.w, .h=r.h};
        chunk.px = (rgba*) malloc(sizeof(rgba) * r.w * r.h);
        for (u32 y = 0; y < r.h; ++y) {
            for (u32 x = 0; x < r.w; ++x) {
                chunk.px[y * r.w + x] = task.px[(r.y + y) * task.w + (r.x + x)];
            }
        }

        const r64 alpha = 0.005;
        image initial = {.w=r.w, .h=r.h, .px=(rgba*)&r.px[0]};
        r64 cost1 = alpha * image_dist(initial, chunk);

        rgba bg = image_nelder_mead(chunk);
        r64 cost2 = 5 + alpha * image_dist(chunk, bg);
        if (cost2 < cost1) {
...
solve(prog_state& state, const image& task) {
    solve_block(state, state.root, {}, task);
}
...
main(int argc, char const *argv[]) {
    ...
    u32 score = _Program.cost + round(imdiff * 0.005);
